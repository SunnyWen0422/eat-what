...
│       ├── interceptor/    # AuthInterceptor（JWT认证拦截器）
│       ├── exception/      # GlobalExceptionHandler
│       └── util/           # TokenUtil(JWT), WeChatUtil, DateUtil
├── recommend-service/  # Python 推荐/AI服务
│   ├── main.py             # FastAPI入口（/chat, /chat/sync, /health）
│   ├── rag.py              # 知识库（31k菜品+7k食材索引，jieba检索）
│   ├── chat_handler.py     # 对话引擎（意图识别+RAG+DeepSeek）
│   ├── db.py               # MySQL连接（pymysql）
│   ├── config.py           # 配置（DeepSeek Key, DB连接）
│   ├── graph/              # 旧LangGraph推荐（闲置）
...
### 菜品
- `GET  /api/dishes?type=X&page=1&pageSize=50` — 分页获取菜品
...
### 推荐
- `POST /api/recommend` — 生成推荐方案（传入meat/veg/soup/dessert/staple数量）
...
1. **Token认证**: 自实现JWT（javax.crypto.HMAC-SHA256），不依赖外部JWT库
2. **推荐算法**: 前端有本地规则引擎（utils/recommend.js），后端有Java规则引擎（RecommendService），AI有DeepSeek对话推荐
3. **前后端推荐降级链**: 前端缓存 → 后端RecommendService → 前端本地规则 → 30道默认菜回退
...
8. **CSS**: 不使用CSS自定义属性（微信不支持），全站主色 #2BA471，圆角14rpx
